...
const Dashboard = () => {
  ...
  const handleAnalyze = (comment: Comment) => {
    ...
    const analyzedComment = {
      ...comment,
      sentiment: comment.sentiment || (["positive", "negative", "neutral"][Math.floor(Math.random() * 3)] as Comment["sentiment"]),
    };
    
    // Update the comment in the list
    setComments(prev => 
      prev.map(c => c.id === comment.id ? analyzedComment : c)
    );
    
    setView({ type: "sentiment", comment: analyzedComment });
  };
  ...
  const filteredComments = comments.filter(comment => {
    ...
    const matchesFilter = !filterSentiment || comment.sentiment === filterSentiment;
  ...
  if (view.type === "sentiment") {
    return <SentimentResult comment={view.comment} onBack={() => setView({ type: "dashboard" })} />;
  }

  if (view.type === "stats") {
    return <StatsDetail type={view.statsType} comments={comments} onBack={() => setView({ type: "dashboard" })} />;
  }
      ...
      <main className="container mx-auto px-6 py-8">
        ...
        <section>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <h2 className="text-lg font-semibold text-foreground">Customer Comments</h2>
